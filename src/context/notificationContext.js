...
export default function NotificationProvider({ children }) {
    ...
    try {
      ...
      const res = await db.listDocuments(
        ...
        [
          Query.equal(
            "userId",
            JSON.parse(localStorage.getItem("Karyakram-user"))?.$id
          ),
  ...
  useEffect(() => {
    client.subscribe(
      `databases.${process.env.REACT_APP_DATABASE_ID}.collections.${process.env.REACT_APP_NOTIFICATIONS_COLLECTION_ID}.documents`,
      (res) => {
        if (
          res.payload?.userId !==
          JSON.parse(localStorage.getItem("Karyakram-user"))?.$id
