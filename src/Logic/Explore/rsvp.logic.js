...
export default function RsvpLogic(event) {
  ...
  const KaryakramUser = JSON.parse(localStorage.getItem("Karyakram-user"));
  ...
  const checkUserIsOwner = () => {
    if (token && cookieFallback && KaryakramUser) {
      if (event?.createdBy === KaryakramUser?.$id) {
        return true;
      }
    }
    return false;
  ...
  const approveRsvp = async (user) => {
    ...
    try {
      ...
      await sendNotification({
        userId: userId,
        fromUserId: KaryakramUser?.$id,
        fromUserName: KaryakramUser?.name,
        type: "RSVP_APPROVED",
  ...
  const rejectRsvp = async (user) => {
    ...
    try {
      ...
      if (membershipId) {
        const teams = new Teams(client);
        res = await teams.deleteMembership(teamId, membershipId);
      }
